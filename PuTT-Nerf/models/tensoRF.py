...
class TensorVM(TensorBase):
    ...
    def init_svd_volume(self, res, device):
        self.plane_coef = torch.nn.Parameter(
            self.init_scale * torch.randn((3, self.app_n_comp + self.density_n_comp, res, res), device=device))
        self.line_coef = torch.nn.Parameter(
            self.init_scale * torch.randn((3, self.app_n_comp + self.density_n_comp, res, 1), device=device))
...
class TensorVMSplit(TensorBase):
    ...
    def init_svd_volume(self, res, device):
        self.density_plane, self.density_line = self.init_one_svd(self.density_n_comp, self.gridSize, self.init_scale, device)
        self.app_plane, self.app_line = self.init_one_svd(self.app_n_comp, self.gridSize, self.init_scale, device)
    ...
    def get_compression_values(self):
        ...
        # Initialize num_compressed_params to 0
        num_compressed_params = 0

        # Calculate the number of parameters for self.density_plane and self.density_line
        for tensor in [self.density_plane, self.density_line]:
            num_parameters = sum(p.numel() for p in tensor.parameters())
            num_compressed_params += num_parameters

        # Calculate the number of parameters for self.app_plane and self.app_line
        for tensor in [self.app_plane, self.app_line]:
            num_parameters = sum(p.numel() for p in tensor.parameters())
            num_compressed_params += num_parameters

        # Calculate the number of parameters for self.basis_mat
        num_parameters = sum(p.numel() for p in self.basis_mat.parameters())
        num_compressed_params += num_parameters
        ...
        # get size in GB
        float_type = self.app_line[0].dtype
...
class TensorCP(TensorBase):
    ...
    def get_compression_values(self):
        ...
        for idx in range(len(self.app_line)):
            # num_compressed_params = num_compressed_params + torch.prod(self.app_line[idx].shape)
            num_compressed_params = num_compressed_params + torch.prod(torch.tensor(self.app_line[idx].shape))
        # self.num_compressed_params = num_compressed_params + torch.prod(self.basis_mat.weight.shape)
        self.num_compressed_params = num_compressed_params + torch.prod(torch.tensor(self.basis_mat.weight.shape))

        self.compression_factor = self.num_uncompressed_params / num_compressed_params
        ...
        self.sz_uncompressed_gb = self.num_uncompressed_params * bytes_per_float / 1024**3
        self.sz_compressed_gb = self.num_compressed_params * bytes_per_float / 1024 ** 3
