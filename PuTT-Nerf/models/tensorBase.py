...
class TensorBase(torch.nn.Module):
    def __init__(self, aabb, gridSize, device, density_n_comp = 8, appearance_n_comp = 24, app_dim = 27,
        ...
        self.should_shrink = should_shrink
        self.max_rank_density = max_rank_density 
        self.max_rank_appearance = max_rank_appearance
    ...
    def update_stepSize(self, gridSize):
        ...
        self.radius = (self.gridSize-1) * 0.5
    ...
    @torch.no_grad()
    def filtering_rays(self, all_rays, all_rgbs, N_samples=256, chunk=10240*5, bbox_only=False, wandb=None):
    ...
    def forward(self, rays_chunk, white_bg=True, is_train=False, ndc_ray=False, N_samples=-1):
        ...
        if ray_valid.any():
            ...
            if self.fused:
                ...
                sigma_feature = self.compute_densityfeature(points_to_sample)
        ...
        if app_mask.any():
            points_to_sample = xyz_sampled[app_mask]
            if self.name == "TensorTT" or self.name == "TensorRing":
                points_to_sample = self.scale_xyz(points_to_sample)
            ...
            if self.fused:
                ...
                app_features = self.compute_appfeature(points_to_sample)
                valid_rgbs = self.renderModule(points_to_sample, viewdirs[app_mask], app_features)
                rgb[app_mask] = valid_rgbs
